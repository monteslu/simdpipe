...
// Relaxed-SIMD fused multiply-add in JIT kernels. Default ON (≤1 LSB — one
// rounding per a*b+c instead of two; the SP_FAST fidelity-for-speed bargain).
// Set SP_FMA=0 to emit discrete mul+add for byte-exact reproduction.
const FMA = process.env.SP_FMA !== '0';

// Coarse (variable-rate) shading: evaluate the fragment kernel once per 2×1 pixel
// pair — gather the even pixels of two adjacent 4-px groups into one representative
// vector, run the heavy math (the sins) ONCE for 8 pixels, then expand each result
// to its right-hand neighbor. Halves transcendental throughput. This is a VISIBLE
// fidelity drop (half horizontal shading rate — like hardware variable-rate shading
// at 2×1), so unlike SP_FAST/SP_FMA it is OFF by default; opt in with SP_COARSE=1
// (or per-program via createJITProgram(src,{coarse:true})). The varyings still
// interpolate per-pixel; only the kernel evaluation is decimated, so flat/linear
// shaders are unaffected and smooth shaders blur slightly along x.
const COARSE_ENV = process.env.SP_COARSE === '1';
...
export function compileWASM(src, opts = {}) {
  ...
  // Coarse (2×1 VRS) shading: opt-in (SP_COARSE=1 env or {coarse:true}) and only for
  // non-texture kernels — the win is on transcendental-heavy fragment math, and the
  // coarse expand would fight the per-pixel texel gather. Falls back to per-pixel.
  g.coarse = (COARSE_ENV || !!opts.coarse) && !needsTexture;
  let bytes;
  try { bytes = g.build(ast); } catch (e) { return { supported: false, reason: 'codegen: ' + e.message }; }
  return { supported: true, bytes, uniformOrder, uniforms, needsTexture, needsUV, needsColor, coarse: g.coarse };
}
...
class Gen {
  ...
  /* Coarse (2×1 variable-rate) pair body: evaluate the kernel ONCE on the gathered
   * even-pixel representatives of two groups (L_off, L_off2), then expand+store to
   * both. cmA/cmB are the two groups' cover masks. The expensive sins run on 4 lanes
   * standing in for 8 pixels — half the transcendental work. */
  emitCoarsePair(cmA, cmB) {
    this.coarseGather = true;
    const rgba = this.emit(this.ast);                 // loadPlane now gathers evens
    this.coarseGather = false;
    const comp = (i, def) => rgba[i] != null ? rgba[i] : (def != null ? this.constV(def) : this.constV(0));
    const R = comp(0, 0), G = comp(1, 0), B = comp(2, 0), A = rgba[3] != null ? rgba[3] : this.constV(1);
    const packed = this.packRGBA(R, G, B, A);          // [p0,p1,p2,p3] one per pair
    // store to each group: expand the packed result to its 4 px, cover-merge, write.
    const store = (off, cm, half) => {
      const exp = this.expandCoarse(packed, half);
      const dst = this.newLocal(V128);
      this.b(OP.local_get, ...E.uleb(7), OP.local_get, ...E.uleb(off), OP.i32_add);
      this.b(simd(SIMD.v128_load, 0x04, 0x00)); this.b(OP.local_set, ...E.uleb(dst));
      const outc = this.newLocal(V128);
      this.b(OP.local_get, ...E.uleb(exp), OP.local_get, ...E.uleb(dst), OP.local_get, ...E.uleb(cm));
      this.b(simd(SIMD.v128_bitselect)); this.b(OP.local_set, ...E.uleb(outc));
      this.b(OP.local_get, ...E.uleb(7), OP.local_get, ...E.uleb(off), OP.i32_add);
      this.b(OP.local_get, ...E.uleb(outc));
      this.b(simd(SIMD.v128_store, 0x04, 0x00));
    };
    store(this.L_off, cmA, 0);
    store(this.L_off2, cmB, 1);
  }
  ...
  build(ast) {
    ...
    if (this.coarse) { this.buildCoarse(); return buildModule({ params: this.params, results: [], locals: this.locals, body: this.body, exportName: 'run' }); }
  ...
  /* Coarse (2×1 VRS) main loop: step group index by 2, evaluate the kernel ONCE per
   * group-pair (8 px) on the gathered even-pixel representatives, expand+store to both
   * groups. A trailing odd group (n4 odd) falls back to the per-pixel emitGroupBody.
   * Halves transcendental throughput; visible as half-rate horizontal shading. */
  buildCoarse() {
    this.L_off2 = this.newLocal(T.i32);
    const idxA = this.newLocal(T.i32), idxB = this.newLocal(T.i32);
    this.b(OP.i32_const, ...E.sleb(0), OP.local_set, ...E.uleb(this.L_i));
    this.b(OP.block, 0x40); this.b(OP.loop, 0x40);
    // while (i+1 < n4): a full pair is in range
    this.b(OP.local_get, ...E.uleb(this.L_i), OP.i32_const, ...E.sleb(1), OP.i32_add, OP.local_get, ...E.uleb(8), OP.i32_ge_s, OP.br_if, ...E.uleb(1));
    // idxA=i, idxB=i+1 ; L_off=i*16, L_off2=(i+1)*16
    this.b(OP.local_get, ...E.uleb(this.L_i), OP.local_set, ...E.uleb(idxA));
    this.b(OP.local_get, ...E.uleb(this.L_i), OP.i32_const, ...E.sleb(1), OP.i32_add, OP.local_set, ...E.uleb(idxB));
    this.b(OP.local_get, ...E.uleb(idxA), OP.i32_const, ...E.sleb(16), OP.i32_mul, OP.local_set, ...E.uleb(this.L_off));
    this.b(OP.local_get, ...E.uleb(idxB), OP.i32_const, ...E.sleb(16), OP.i32_mul, OP.local_set, ...E.uleb(this.L_off2));
    const cmA = this.loadCoverMask(idxA), cmB = this.loadCoverMask(idxB);
    this.b(OP.block, 0x40);                                   // skip pair if NEITHER covered
    this.b(OP.local_get, ...E.uleb(cmA)); this.b(simd(SIMD.v128_any_true));
    this.b(OP.local_get, ...E.uleb(cmB)); this.b(simd(SIMD.v128_any_true)); this.b(OP.i32_or);
    this.b(OP.i32_eqz, OP.br_if, ...E.uleb(0));
    this.emitCoarsePair(cmA, cmB);
    this.b(OP.end);
    this.b(OP.local_get, ...E.uleb(this.L_i), OP.i32_const, ...E.sleb(2), OP.i32_add, OP.local_set, ...E.uleb(this.L_i));
    this.b(OP.br, ...E.uleb(0));
    this.b(OP.end); this.b(OP.end);

    // trailing odd group: per-pixel fallback (i < n4, at most one iteration)
    this.b(OP.block, 0x40); this.b(OP.loop, 0x40);
    this.b(OP.local_get, ...E.uleb(this.L_i), OP.local_get, ...E.uleb(8), OP.i32_ge_s, OP.br_if, ...E.uleb(1));
    this.b(OP.local_get, ...E.uleb(this.L_i), OP.i32_const, ...E.sleb(16), OP.i32_mul, OP.local_set, ...E.uleb(this.L_off));
    const cmR = this.loadCoverMask(this.L_i);
  ...
  loadPlane(paramIdx) {
    if (this.coarseGather) return this.loadPlaneCoarse(paramIdx);
    const L = this.newLocal(V128);
  ...
  // Coarse load: gather the EVEN pixels of two adjacent 4-px groups (at L_off and
  // L_off2) into one representative vector [A0, A2, B0, B2] via i8x16.shuffle. The
  // kernel then evaluates 4 representatives that stand in for 8 pixels (each even px
  // also covers its right neighbor) → the sins run once per pair instead of per px.
  loadPlaneCoarse(paramIdx) {
    const a = this.newLocal(V128);
    this.b(OP.local_get, ...E.uleb(paramIdx), OP.local_get, ...E.uleb(this.L_off), OP.i32_add);
    this.b(simd(SIMD.v128_load, 0x04, 0x00)); this.b(OP.local_set, ...E.uleb(a));
    const b = this.newLocal(V128);
    this.b(OP.local_get, ...E.uleb(paramIdx), OP.local_get, ...E.uleb(this.L_off2), OP.i32_add);
    this.b(simd(SIMD.v128_load, 0x04, 0x00)); this.b(OP.local_set, ...E.uleb(b));
    // result = [A.lane0, A.lane2, B.lane0, B.lane2]; bytes from A (0..15) then B (16..31)
    const L = this.newLocal(V128);
    this.b(OP.local_get, ...E.uleb(a), OP.local_get, ...E.uleb(b));
    this.b(simd(SIMD.i8x16_shuffle, 0,1,2,3, 8,9,10,11, 16,17,18,19, 24,25,26,27));
    this.b(OP.local_set, ...E.uleb(L));
    return L;
  }

  // Expand a coarse result vector [r0,r1,r2,r3] back to the two 4-px groups:
  //  group A (left pair of pairs)  = [r0,r0,r1,r1]
  //  group B (right pair of pairs) = [r2,r2,r3,r3]
  // via i8x16.shuffle (single operand duplicated). half=0 → A, half=1 → B.
  expandCoarse(v, half) {
    const L = this.newLocal(V128);
    this.b(OP.local_get, ...E.uleb(v), OP.local_get, ...E.uleb(v));
    if (half === 0) this.b(simd(SIMD.i8x16_shuffle, 0,1,2,3, 0,1,2,3, 4,5,6,7, 4,5,6,7));
    else            this.b(simd(SIMD.i8x16_shuffle, 8,9,10,11, 8,9,10,11, 12,13,14,15, 12,13,14,15));
    this.b(OP.local_set, ...E.uleb(L));
    return L;
  }
  ...
  // Fused multiply-add a*b+c into a fresh local (relaxed-SIMD, one rounding).
  // Falls back to discrete mul+add when SP_FMA is off (byte-exact mode).
  madd(a, b, c) {
    if (!FMA) return this.v2(SIMD.f32x4_add, this.v2(SIMD.f32x4_mul, a, b), c);
    const L = this.newLocal(V128);
    this.b(OP.local_get, ...E.uleb(a), OP.local_get, ...E.uleb(b), OP.local_get, ...E.uleb(c));
    this.b(simd(SIMD.f32x4_relaxed_madd)); this.b(OP.local_set, ...E.uleb(L));
    return L;
  }
  // Fused -(a*b)+c = c - a*b.
  nmadd(a, b, c) {
    if (!FMA) return this.v2(SIMD.f32x4_sub, c, this.v2(SIMD.f32x4_mul, a, b));
    const L = this.newLocal(V128);
    this.b(OP.local_get, ...E.uleb(a), OP.local_get, ...E.uleb(b), OP.local_get, ...E.uleb(c));
    this.b(simd(SIMD.f32x4_relaxed_nmadd)); this.b(OP.local_set, ...E.uleb(L));
    return L;
  }
  ...
  emit(node) {
    switch (node.op) {
      ...
      case 'bin': return this.emitBinFused(node);
  ...
  // Broadcast a per-channel value array to width n (scalar→vector splat-by-reuse).
  bcv(x, n) { return x.length === 1 && n > 1 ? new Array(n).fill(x[0]) : x; }

  // Fuse a*b±c into a single relaxed FMA when an add/sub has a multiply operand.
  // This collapses the dense mul+add chains (Horner polys, mix, 0.5+0.5*sin) that
  // dominate heavy shaders into one fused op each — fewer ops, one rounding (≤1 LSB).
  emitBinFused(node) {
    const isMul = (n) => n && n.op === 'bin' && n.o === '*';
    if (FMA && node.o === '+') {
      // a + (m*n)  or  (m*n) + a
      if (isMul(node.b)) return this.fma(node.b.a, node.b.b, node.a, false);
      if (isMul(node.a)) return this.fma(node.a.a, node.a.b, node.b, false);
    } else if (FMA && node.o === '-') {
      // a - (m*n)  →  -(m*n) + a
      if (isMul(node.b)) return this.fma(node.b.a, node.b.b, node.a, true);
    }
    return this.emitBin(node.o, this.emit(node.a), this.emit(node.b));
  }

  // Emit fused m*n (+/-) c, where m,n,c are AST nodes. neg=true → c - m*n.
  fma(mNode, nNode, cNode, neg) {
    let M = this.emit(mNode), N = this.emit(nNode), C = this.emit(cNode);
    const w = Math.max(M.length, N.length, C.length);
    M = this.bcv(M, w); N = this.bcv(N, w); C = this.bcv(C, w);
    return M.map((m, i) => neg ? this.nmadd(m, N[i], C[i]) : this.madd(m, N[i], C[i]));
  }
  ...
  emitSin(x) {
    ...
    // xr = x - fl*2PI  → one fused negative-multiply-add
    const xr = this.nmadd(fl, this.constV(2 * Math.PI), x);
    // minimax: sin(x) ~ x*(0.98793 - 0.15545*x^2 + 0.00565*x^4)  (x in [-PI,PI])
    // Horner via fused madds: poly = (c2*x2 + c1)*x2 + c0
    const x2 = this.v2(SIMD.f32x4_mul, xr, xr);
    const c4 = this.madd(x2, this.constV(0.0056549), this.constV(-0.1554991));
    const poly = this.madd(x2, c4, this.constV(0.9879227));
