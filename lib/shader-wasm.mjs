...
export function compileWASM(src, opts = {}) {
  ...
  return { supported: true, bytes, uniformOrder, uniforms, needsTexture };
}
...
class Gen {
  ...
  build(ast) {
    ...
    // Coverage mask FIRST, then an early-skip: if no lane of this 4-group is
    // covered, branch straight to the loop increment — the fragment expression
    // (which for a heavy shader is several sin/pow/texture ops) is the dominant
    // cost, so not evaluating it on fully-uncovered groups is a direct win on any
    // scene that doesn't fill the framebuffer. Output is unchanged: an all-zero
    // cover mask would have bitselect'd the background back in anyway. Wrap the
    // shade+store in an inner block we can `br_if 0` over.
    const coverMask = this.loadCoverMask();
    this.b(OP.block, 0x40);                                  // inner skip block (br depth 0 == here)
    // skip the shade body when no lane is covered: i32.eqz(v128.any_true(mask)) → br_if 0
    this.b(OP.local_get, ...E.uleb(coverMask));
    this.b(simd(SIMD.v128_any_true));
    ...
    // bitselect packed vs the existing color so uncovered lanes keep the background.
    ...
    this.b(OP.end);                                         // end inner skip block
